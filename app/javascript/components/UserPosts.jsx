...
const UserPosts = ({user, isCurrentUser, posts, currentUser, maxItemCount}) => {
    ...
    <UserSideMenuWrapper user={user} isCurrentUser={isCurrentUser}>
      <Box sx={{ minHeight: '100%', bgcolor: '#f5f5f5' }}>
        <Container maxWidth="md" sx={{ py: '40px' }}>
          <Typography variant={matches ? "h5" : "h6"} align="center">投稿一覧</Typography>
          {totalItemCount == 0
          ...
          <Stack spacing={2}>
            {displayedItems.map((post, i) => (
              <CustomPaper component="a" href={"/posts/"+post.id} elevation={3} key={i} id={"my-posts-"+i}>
                <Box sx={{ width: { xs: '40%', sm: '35%', md: '30%' }, p: { xs: '12px 4px', sm: '4px' } }}>
